          ...
          {studyProgrammes.map(programme => (
            <div className={style.boxes}>
              <div key={programme.node.code}>
                ...
                <a className={style.social} href={programme.node.instagram} target='_blank' rel="noreferrer">@instagram</a>
                <p>{programme.node.description[0].children[0].text}</p>
                <Link to={`${programme.node.slug.current}`}><button>Til studenter</button></Link>
              </div>
            </div>
          ))}
        </div>
